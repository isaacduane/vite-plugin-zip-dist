 ...
 * @LastEditors: LYM
 * @LastEditTime: 2022-06-23 10:28:17
 * @Description: dist文件压缩
 */
 ...
 * @param dayjsFormat 默认值 "YYMMDD_HHmm" ; dayjs 日期格式  YYMMDD_HHmm,会拼在压缩包名称上,便于区分压缩包 备注：不传或者传空字符串压缩文件名不带时间
 * @param includeDistDir 默认值 false ; 为true时  压缩包会包含dist 目录本身,  false 只压缩dist 内容
 */
export default function ({
  zipName = "dist",
  zipDir = "",
  dayjsFormat = "YYMMDD_HHmm",
  includeDistDir = false,
} = {}) {
  let viteConfig;
  return {
    ...
    closeBundle() {
      ...
      let zipFullName = ''
      if(dayjsFormat) {
        zipFullName = `${zipName}_${nowDateTime.format(dayjsFormat)}.zip`;
      } else {
        zipFullName = `${zipName}.zip`;
      }

      const trueZipDir = zipDir
        ? path.resolve(viteConfig.root, zipDir)
        : distDir;
